...
| DesktopVK (default on Mac/Linux) | `net8.0` | Native + Mac/Linux.Vulkan **3.8.5** | `libSDNative` + Autodesk FBX 2020.3.7 (macOS arm64); Linux FBX TBD | MonoGame/FAudio |
...
- macOS DesktopVK links Autodesk FBX SDK **2020.3.7** (`SDNative/3rdparty/fbxsdk/macos/libfbxsdk.dylib`) — same `Mesh_Fbx` / `FbxToOpenGL` path as Windows. Fetch/refresh: `bash scripts/fetch-fbxsdk-macos.sh`. Assimp `.obj` sidecars remain an optional fallback only. Linux FBX runtime still TBD.
